...
function setHintRules() {
	...
	ss.insertRule('a[highlight=hint_elem]   {color:black !important;background-color:yellow !important}', 0);
	ss.insertRule('a[highlight=hint_active] {color:black !important;background-color:lime   !important}', 0);
}
	...
	for (var i = 0; i < elems.length; i++) {
		...
		if ( elem_bottom >= win_top && elem_top <= win_bottom) {
			...
			span.style.cssText = [
				...
				'position: absolute;',
				'font-size: 13px;',
				'background-color: ' + (hint_open_in_new_tab ? '#FF7F00' : 'red') + ' !important;',
